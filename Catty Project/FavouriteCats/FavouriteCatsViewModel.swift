...
final class FavouriteCatsViewModel {
    ...
    init() {
        ...
        dataProvider.favImages.subscribe(onNext: { [weak self] images in
            ...
            var mapped = images.map { CatCellViewModel(id: $0.image.id, image_url: $0.image.url, width: $0.image.width, height: $0.image.height, favouriteId: "\($0.id)", isFavourite: true, isFavouriteController: true) }
